...
fn benchmark(c: &mut Criterion) {
    ...
    let ray_points: Vec<Point2<f64>> = (0..100)
        .into_iter()
        .map(|i| Point2::new(i as f64 / 10.0, 0.0))
        .collect();
    let rays: Vec<Ray<f64>> = ray_points
        .iter()
        .map(|p| Ray::new(*p, Vector2::new(0.1, 0.9)))
        .collect();

    c.bench_function("Polyline Intersections", |b| {
        b.iter(|| polyline_intersections(black_box(&line), black_box(&rays)))
    });
}
