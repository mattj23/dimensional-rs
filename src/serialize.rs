...
impl VectorList2f64 {
    pub fn from_vectors(vectors: &[Vector2<f64>]) -> Self {
        VectorList2f64 { vectors: vectors.to_vec() }
    }

    pub fn from_points(points: &[Point2<f64>]) -> Self {
        VectorList2f64 { vectors: points.iter().map(|p| p.coords).collect() }
    }
}
...
mod tests {
    ...
    fn test_serialize_veclist() {
        let vecs = vec![Vector2::new(0.0, 1.0), Vector2::new(2.0, 3.0), Vector2::new(4.0, 5.0)];
